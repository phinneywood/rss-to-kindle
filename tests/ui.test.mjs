...
test('Your editor reviews fixed rules and saves both editable guidance fields',async()=>{
  const result=await run(`dashboard();document.querySelector('#open-editor').click();const fixed=modal.textContent,form=document.querySelector('#editor-settings-form');document.querySelector('#editorial-brief').value='Architecture, cities, and deeply reported essays.';document.querySelector('#editorial-instructions').value='Prefer primary sources and essays over quick takes.';document.querySelector('#editorial-instructions').dispatchEvent(new Event('input',{bubbles:true}));const effective=document.querySelector('#effective-editor').textContent;let request;api=async(path,options)=>{request={path,body:options.body};state.settings={...state.settings,...options.body};return state};await form.onsubmit({preventDefault(){},currentTarget:form});return {request,fixed,effective,home:document.querySelector('.editor-card').textContent}`);
  assert.equal(result.request.path,'/settings');assert.equal(result.request.body.editorial_brief,'Architecture, cities, and deeply reported essays.');assert.equal(result.request.body.editorial_instructions,'Prefer primary sources and essays over quick takes.');assert.match(result.fixed,/Include every eligible subscribed article exactly once/);assert.match(result.fixed,/Reviewable, but not editable/);assert.match(result.effective,/Prefer primary sources and essays over quick takes/);assert.match(result.home,/Customized/);
});
